function y = gmp(x, Model)
% y = gmp(x, Model)
% gmp applies the generalized memory polynomial model given the 
% appropriate Coefficients and corresponding orders. This model is divided
% into three separate expressions, the first one is the memory polynomial
% model i.e., terms on the diagonal, the second [third] one is for lagging 
% [leading] envelope cross terms.
% 
% Input Arguments:
% x     : time domain input signal.
% Model :  structure with 5 fields:
    % NL    : NonLinearity, 1x3 structure array, 
        % NL(1) is for the MP model and the field inside is Kav the vector of 
        % indices for MP,
        % NL(2) [NL(3)] is for the second [third] nonlinearity and the field inside
        % is Kbv [Kcv] vector of indices for lagging [leading] envelope cross terms.
    % Mem   : Memory, 1x3 structure array,
        % Mem(1) is the memory for MP, with one field Lav a vector of indices for
        % memory.
        ...
        % of memory.
    % Coeff : vector of all the Coefficients akl, bklm and cklm with the
    ...
    % Type  : String to define the type of the model 
    %           'mp' or 'gmp'
% Symmetric : A string, if equal to one the parameters to control the
    % leading terms don't have to be included, they will be chosen equal to
    % those of lagging terms.
...
for k=1:length(Kav)
    for l=1:length(Lav)
        ...
        A1= [A1 cv.*(abs(cv).^(Kav(k)))]; %#ok<*AGROW>
    end
end
